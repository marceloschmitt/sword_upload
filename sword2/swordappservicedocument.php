...
class SWORDAPPServiceDocument {
	...
	function __construct($sac_theurl, $sac_newstatus, $sac_thexml = '') {
		...
		if ($sac_thexml != '') {
        //print_r($sac_thexml); exit;
        //$file = fopen('teste.xml','a+');
        //fwrite($file,$sac_thexml);
        //fclose($file);
          //  exit;
            //echo strlen($sac_thexml); exit;
           // print_r($sac_thexml); exit;


            //$sac_thexml = mb_substr($sac_thexml, 0,null,'utf-8');
            //$sac_thexml = <<< EOF
           /* <?xml version="1.0" encoding="UTF-8"?>{$sac_thexml}*/
// EOF;
           // print_r($sac_thexml); exit;
